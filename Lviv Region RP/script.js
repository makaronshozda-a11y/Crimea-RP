...
(function initTabs() {
  const buttons = document.querySelectorAll(".tab-btn:not(.gear-btn)");
  ...
  function restartCardAnimations(container) {
    ...
    const cards = Array.from(container.querySelectorAll(
      ".rule-card, .role-card, .license-check-box, .review-card"
    ));
...
// ==================== ЛІЦЕНЗІЇ ====================
// Без вибору категорії заздалегідь - один рядок пошуку за ніком або номером
// ліцензії одразу по всій базі. Результат показує ВСІ знайдені ліцензії гравця
// (може бути кілька типів одразу).
(function initLicenses() {
  ...
  if (!nickInput || typeof licensesData === "undefined") return;

  function renderLicenseCard(lic) {
    return `
      <div class="license-result-card">
        <span class="license-result-status">Ліцензія дійсна</span>
        <div class="license-result-row"><span class="license-result-label">Нік гравця</span><span class="license-result-value">${lic.nick}</span></div>
        <div class="license-result-row"><span class="license-result-label">Тип ліцензії</span><span class="license-result-value">${licenseTypeNames[lic.type] || lic.type}</span></div>
        <div class="license-result-row"><span class="license-result-label">Термін дії</span><span class="license-result-value">${lic.validFrom} — ${lic.validTo}</span></div>
      </div>
    `;
  }
  ...
  function checkLicense() {
    ...
    if (!query) {
      result.innerHTML = `<p class="placeholder-text">Введіть username гравця Roblox.</p>`;
      return;
    }

    const found = licensesData.filter(lic =>
      lic.nick.toLowerCase() === query
    );

    if (found.length > 0) {
      result.innerHTML = `<div class="license-results-list">${found.map(renderLicenseCard).join("")}</div>`;
    } else {
      result.innerHTML = `
        <div class="license-result-card invalid">
          <span class="license-result-status invalid">Ліцензію не знайдено</span>
          <div class="license-result-row"><span class="license-result-label">Запит</span><span class="license-result-value">${nickInput.value.trim()}</span></div>
        </div>
      `;
    }
  }
...
// ==================== ШЕСТЕРНЯ (код сервера + приєднання + посилання) ====================
(function initGearMenu() {
  ...
  if (!gearBtn || !gearOverlay) return;

  function openGear() {
    gearOverlay.classList.add("active");
  }

  function closeGear() {
    gearOverlay.classList.remove("active");
  }

  gearBtn.addEventListener("click", openGear);
  gearClose.addEventListener("click", closeGear);
  gearOverlay.addEventListener("click", (e) => {
    if (e.target === gearOverlay) closeGear();
  });
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && gearOverlay.classList.contains("active")) closeGear();
  });
})();
...
(() => {
  ...
  document.querySelectorAll(
    ".review-card,.license-result-card,.rules-grid,.roles-grid"
  ).forEach(el => {
